...
public class RouteGuideClient {
    ...
    private final RoutedGuideGrpc.RoutedGuideBlockingStub blockingStub;

    private final RoutedGuideGrpc.RoutedGuideStub asyncStub;
    ...
    public RouteGuideClient(Channel channel) {
        this.blockingStub = RoutedGuideGrpc.newBlockingStub(channel);
        this.asyncStub = RoutedGuideGrpc.newStub(channel);
    }
    ...
    public void recordRoute(List<Feature> features, int numPoints) throws InterruptedException {
        info("*** RecordRoute");
        final CountDownLatch finshLatch = new CountDownLatch(1);
        StreamObserver<RouteSummary> responseObserver = new StreamObserver<RouteSummary>() {
            @Override
            public void onNext(RouteSummary routeSummary) {
                info("Finished trip with {0} pints. Passed {1} features. travelled {2} meters. It took {3} seconds.",
                        routeSummary.getPointCount(), routeSummary.getFeatureCount(), routeSummary.getDistance(),
        ...
        try{
            // Send numPoints pints randomly selected from features list.
        ...
        if (!finshLatch.await(1, TimeUnit.MINUTES)) {
    ...
    public CountDownLatch routeChat() {
        ...
        StreamObserver<RouteNote> requestObserver = asyncStub.routeChat(new StreamObserver<RouteNote>() {
            ...
            public void onNext(RouteNote routeNote) {
                info("Got message \"{0}\" at {1}, {2}", routeNote.getMessage(), routeNote.getLocation().getLatitude(),
            ...
            @Override
            public void onError(Throwable throwable) {
                warning("RouteChat Filed: {0}", Status.fromThrowable(throwable));
                if (testHelper != null) {
                    testHelper.onRpcError(throwable);
                }
    ...
    interface TestHelper {
        /**
         * Used for verify/inspect message received form server.
         */
        void onMessage(Message message);

        /**
         * Used for verify/inspect error received form server.
         */
